...
marketSubscription c new =
  case (fmap snd .
        headMay .
        HashMap.toDescList .
        HashMap.filter isJust .
        HashMap.map (sameAsNewMarketSubscribeRequests new) . ssRequests . cState) c of
...
orderSubscription c new =
  case (fmap snd .
        ...
        HashMap.toDescList .
        HashMap.filter isJust .
        HashMap.map (sameAsNewOrderSubscribeRequests new) . ssRequests . cState) c of
    Nothing    -> request c new
    (Just old) -> resendOldRequest c old
