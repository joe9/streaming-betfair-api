...
heartbeat c = request c H.defaultHeartbeatMessage
...
marketSubscription c new = do
  ...
  case (lastMay .
        ...
        IntMap.map (sameAsNewMarketSubscribeRequests new) . ssRequests . cState)
         c of
...
sameAsNewMarketSubscribeRequests new (MarketSubscribe old)
  | old {M.id = 0, M.clk = Nothing, M.initialClk = Nothing} ==
      new {M.id = 0, M.clk = Nothing, M.initialClk = Nothing} = Just old
  | otherwise = Nothing
...
orderSubscription c new =
  case (lastMay .
        IntMap.elems .
        IntMap.filter isJust .
        IntMap.map (sameAsNewOrderSubscribeRequests new) . ssRequests . cState)
         c of
...
sameAsNewOrderSubscribeRequests new (OrderSubscribe old)
  | old {O.id = 0, O.clk = Nothing, O.initialClk = Nothing} ==
      new {O.id = 0, O.clk = Nothing, O.initialClk = Nothing} = Just old
  | otherwise = Nothing
...
marketIdsSubscription c mids =
  marketSubscription
    c
    ((M.defaultMarketSubscriptionMessage)
     { M.marketFilter =
         MF.defaultMarketFilter
         {MF.bettingTypes = [BT.ODDS], MF.marketIds = Just mids}
     })
...
bulkMarketsSubscription mf c =
  marketSubscription
    c
    (M.defaultMarketSubscriptionMessage {M.marketFilter = mf})
